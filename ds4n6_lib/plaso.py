...
def plaso_extract_evtx(plobj, nwf=d4.main_nwf, recovered=False, save_xml=False, xml_filename=""):
    ...
    # TODO: Verify if this one works with the Szechuan image
        ...
        if evtxdfs_usepickle :
            if datapath == "":
                ...
                if os.path.exists(datapath):
                    # Save to pickle - - - - - - - - - - - - - - - - - - - - - - - - - -
                    # TODO : save_pickle(evtxdfspklf, evtxdfs, evtxdfs_usepickle, do_harmonize=False)
                    pickle.dump(evtxdfs, open(evtxdfspklf, "wb" ))
                    print("- Done.")

                # TODO: savepathtonb hardcoded by now. It should be an arg.
                savepathtonb = True
                if savepathtonb :
                    ...
                    # TODO: FIX: Saving the notebook via JavaScript does not work at present. 
                    #            According to online documentation it may not even be  possible.
                    #
                    #print("- Saving notebook")
                    # Apparently this only works in Classic Notebook, not JupyterLab
...
def plaso_extract_win_reg_service(plobj):
    ...
    # TODO: Explode values entries into the following fields
    # Common Fields
    df['Group']              = 'd4_niy'
    df['Tag']                = 'd4_niy'
    ...
    # Specific Fields
    # TODO: Dynamically Explode values entries into new columns (like evtx)
...
def simple_func(df, **kwargs):
    ...
    # Arg. Parsing -------------------------------------------------------------
    # TODO: We should change these definitions to ...._opt, and then define
    #       the xxx variable.
    #       This is important because we may want to change the value of the
    #       var only in the option has not been specifically set
    #       (e.g. wrap_cols requires beautify_cols, but we should not 
    #        enable beautify_cols if it has been specifically set to False.
    #        only if it is undefined should be enable it in wrap)
