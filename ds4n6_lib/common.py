...
def read_data_common(evdl, **kwargs):
    ...
    # TODO: Arg: read_compname_from = { filename | foldername }
    ...
    # TODO: This warning is ok for the moment, but we should probably find a 
    #       better/smarter approach
            ...
            # Read File -------------------------------------------------------
                ...
                if do_harmonize and len(df) > 0:
                    if dftype == "pandas_dataframe-"+tool+"-raw":
                        ...
                        #TODO: REVIEW: Not sure if this change breaks something
                        #df = harmonize(df, tool=tool, hostname=hostname, drop_non_ham_cols=drop_non_ham_cols, data_os=data_os)
                        df = harmonize(df, **kwargs)
            ...
            for dir in dirs:
                if dir != evdl:
                    ...
                    if do_harmonize  and len(dfs[cat]) > 0:
                        #TODO: REVIEW: Not sure if this change breaks something
                        #dfs[cat] = harmonize(dfs[cat], tool=tool, drop_non_ham_cols=drop_non_ham_cols, data_os=data_os)
                ...
                for fname in filesuniq:
                    ...
                    if ".csv" in fname:
                        ...
                        if path_prefix != "":
                            ...
                            # TODO: kape-specific - We should generalize this
                            # kape sometimes doubles the "_" to "__" after the drive letter
                        ...
                        if do_harmonize  and len(df) > 0:
                            #TODO: REVIEW: Not sure if this change breaks something
                            #df = harmonize(df, tool=tool, hostname=hostname, category=category, plugin=artbase, drop_non_ham_cols=drop_non_ham_cols, data_os=data_os)
                            if pluginisdfname == True:
...
def save_pickle(evdl, dfs, use_pickle=True, do_harmonize=True):
    # TODO: FIX: Re-document as ham-specific
    """Save a pickle
    Input:
        evdl: evidence path
...
def dict_source_identify_func(dict):
    ...
    # TODO: FIX: REL: This is VERY wrong. We need to identify the
    #                 Least Common Denominator for all elements
...
def df_source_identify_func(df):
    ...
    # TODO: FIX: This should be removed vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    # TODO: Implement an extra Tool column in raw tool output
    # autoruns --------------------------------------------------------------------
    ...
    # TODO: Define how to identify RAW vs HTM
...
#        df = df.rename(columns={col: colnew})
    ...
    # TODO: get_host_from_path option
    ...
    for col in d4_cols:
        if len(df[col].drop_duplicates()) == 1:
            ...
            #       This isna().any() method is very slow.
            #       We should probably td a try/except for the "is None" method,
            #       and it errs then we would execute the isna().any() method
