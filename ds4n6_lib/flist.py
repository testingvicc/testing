...
def analysis_unique_files_folder(exefs, thisexed_path="/Windows/Temp", exef_intg_max_occs="100", compop='==', recurse=False, prevdays=0, tsfield='m', verbose=False):
    # TODO: Analyze carefully and then document
    # TODO:
...
def convert_flist_ham_to_hml(flist_ham_df):
    ...
    # FEATURE SELECTION #################################################################################################
    filtered_flist_ham_df = flist_ham_df.query("TSNTFSAttr_ == 'STD_INFO'")
    hml_df = filtered_flist_ham_df[['Size', 'MACB', 'Permissions_', 'FilePath_']]
    
    # FEATURE ENGINEERING #################################################################################################
    # Rename Columns
    #hml_df = hml_df.rename(columns={'EVOName_': 'Computer_'})
    ...
    hml_df['FilePath__'] = hml_df['FilePath_'].str.replace('/[^\/]*$','')
    hml_df['FileName_'] = hml_df['FilePath_'].str.replace('.*?/','')
