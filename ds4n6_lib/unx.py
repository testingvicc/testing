...
def dfgrep(*args):
    ...
    """
    # TODO: Adapt for multiple cols

    # def syntax():
    #     print('Syntax: dfgrep("<column>","<regex>"[,"<options>"])')
    #     print("        Options: i: case insensitive")
    #     print("                 v: reverse")
    #     print("                 p: explode cols with series elements")
    #     print("                 t: do not apply style (highlight hits)")
    #     print("        If your DF has only 1 column, you can skip the column name,")
    #     print('        just specify ""')
    ...
    for col in cols:
        ...
        # TODO: FIX: [DO NOT FIX YET] issue-229-1-unx This works for some DFs and not others - Investigate - 
        #if type(df[col][0]) == int or type(df[col][0]) == float or isinstance(df[col][0], np.integer):
        #    df[col]=df[col].astype(str)

        #if type(df[col][0]) != str:
        #    if not "p" in opt:
        #        print("WARNING: "+col+" column type is not string, it's "+str(type(df[col][0]))+".")
        #        print("         Try the 'p' option to explode the column.")
