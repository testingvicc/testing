...
def evtid_dfs_build(df):
    ...
    # TODO: 
    # - Merge this function with the evtid_dfs_build_raw function
...
def harmonize(df, **kwargs):
    ...
    # TODO: Define specific log formats at the end and implement here
    #       a routine that applies those formats
    # Security ----------------------------------------------------------------
...
# TODO: [MEDIUM] Implement "merge=True" option, so the different DFs are merged in one single output
...
def analysis_evtxfiles(dfs, type="", argtsmin="", argtsmax=""):
    # TODO: IMPLEMENT argtsmin/argtsmax
    #dftsmin=dfs.index.min()
    #dftsmax=dfs.index.max()
...
def analysis_access_stats(obj, type="", firstdate="", lastdate="", freq="D", detail=True):
    ...
    # TODO: Implement Support for secevtxrawdf
    #secevtsrawdf = obj
    #evts4624raw = secevtsrawdf.query('EventID_ == 4624')
    #evts4624    = evts4624raw.d4evtx.consolidate_columns()

    if d4.debug >= 2:
        print("DEBUG: [DBG"+str(d4.debug)+"] ["+str(os.path.basename(__file__))+"] ["+str(inspect.currentframe().f_code.co_name)+"()]")

    # TODO: Verify if input is Security.evtx and has the necessary evtids (4624, ...)
    ...
    # TODO: Instead of printing side by side, consolidate all info in a single DF
...
# TODO: xray Functions To Be Removed
...
def simple_func(obj, *args, **kwargs):
    # TODO: simple() can be called in two contexts:
    #
    #         1. evtx file DF
    #         2. evtid DF
    #
    #       - Currently only 1. is implemented. Implement 2.
    #       - Actually, since 2 is a subset of 1, it actually is implemented
    #         However, there are couple of things that are failing that must
    #         be fixed: 
    #         + EvtID col is not shown (not in NonDiag, nor in DF)
    #         + All EvtID descriptions are listed
    #
    #----------------------------------------------------------------------
...
def simple_evtx_file_func(df, *args, **kwargs):
    ...
    # TODO: Add support for combinations of (multiple) numbers and (multiple) macros
    ...
    # TODO: This feature should be included in the simple_common() function
    ...
    # TODO: FIX: Merge Statistics and Descriptions in a single output, in a DF
    #            i.e. create an extra column in the Statistics DF called 
    #            "Description"

    # If we have the evtx file registered in the evtid_desc DF, 
    # then "resolve" evtids to descriptions
#    evtids = df['EventID_'].drop_duplicates()
#    evtids_sorted = evtids.sort_values(ascending=True)
#    missingdescs = False
#    anydescs = False
#    if evtxfname in evtid_desc.keys():
#        evtiddscstr=""
#        for evtid in evtids_sorted:
#            if evtid in evtid_desc[evtxfname].keys():
#                evtiddesc = evtid_desc[evtxfname][evtid]
#                evtidblt = "\n  * "+str(evtid)+": "+str(evtiddesc)
#                evtiddscstr = evtiddscstr + evtidblt
...
#        if anydescs  and out_meta:
#            display(Markdown("**EvtIDs Descriptions:**"+evtiddscstr))

#            if missingdescs :
#                display(Markdown("Note: Some EvtIDs don't have an associated description available."))
