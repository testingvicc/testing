...
def insert_or_move_column_after_another_by_name(df, col, refcol, defvalue=None, where2move="after"):
    #TODO: FIX: Not sure if this works. Verify.
    """
...
def collapse_constant_columns(df):
    ...
    if dfnrows != 0:
        ...
        for col in df.columns:
            ...
            if col != 'nan':
                # TODO: Not sure why I introduced this "isinstance(df[col].iloc[0], str)".
                #       Maybe something breaks with some types of data, but we 
                #       certainly need to be able to collapse cols of other types
                #if isinstance(df[col].iloc[0], str) and len(df[col].drop_duplicates()) == 1:
